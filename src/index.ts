...
import { affiliatePartners, monetizationIdeas } from "./monetization.ts";
...
function originalUrl(item: ProductItem): string {
...
function partnerUrl(item: ProductItem): string | undefined {
  return affiliatePartners[inferBrandDomain(item)];
}

function itemUrl(item: ProductItem): string {
  return partnerUrl(item) || originalUrl(item);
}
...
function renderProductCard(item: ProductItem, accent: number, locale: Locale): string {
  ...
  return html`<a class="product-card accent-${accent % 5}" href="${escapeHtml(url)}" target="_blank" rel="nofollow noopener">
    ...
    <footer><span>${escapeHtml(item.visits || domain)}</span><em>${escapeHtml(partnerUrl(item) ? "Partner →" : `${t.visit} →`)}</em></footer>
  </a>`;
}
...
function renderGrowthAndMonetization(locale: Locale): string {
  const isZh = locale === "zh";
  const ideas = monetizationIdeas.map((idea) => html`<li><strong>${idea.rank}. ${escapeHtml(idea.title)}</strong><span>${escapeHtml(isZh ? idea.fit : idea.description)}</span></li>`).join("");
  return html`<section class="growth-box" aria-label="${isZh ? "曝光和变现策略" : "Growth and monetization strategy"}">
    <div><p class="eyebrow">${isZh ? "SEO + Monetization" : "SEO + Monetization"}</p><h2>${isZh ? "先做联盟深链，再叠加广告" : "Affiliate deep links first, ads later"}</h2><p>${isZh ? "当前阶段最可能有效的是联盟/合作伙伴深链：它贴合用户点击意图，不需要先等巨大流量，也不会破坏页面体验。等自然搜索流量稳定后，再加入 AdSense 或赞助卡片。" : "The highest-probability first monetization is affiliate/partner deep links: it matches click intent, works before huge traffic, and keeps the page clean. Add display ads or sponsored cards after organic traffic compounds."}</p></div>
    <ol>${ideas}</ol>
  </section>`;
}
...
function renderPageShell(url: URL, locale: Locale, body: string, extraTitle = ""): Response {
  ...
  const t = dictionary[locale];
  const jsonLd = [{ "@context": "https://schema.org", "@type": "WebSite", name: t.title, url: host, description: t.subtitle, inLanguage: locale === "zh" ? "zh-CN" : "en", potentialAction: { "@type": "SearchAction", target: `${host}/?q={search_term_string}`, "query-input": "required name=search_term_string" } }, { "@context": "https://schema.org", "@type": "ItemList", name: t.title, itemListElement: toolifyData.sections.flatMap((section) => section.items.slice(0, 10).map((item, idx) => ({ "@type": "ListItem", position: idx + 1, name: item.name }))) }];
  ...
  <meta name="keywords" content="AI tools, AI ranking, AI tool directory, AI products, AI SaaS, micro SaaS, affiliate AI tools, best AI products" />
...
function renderHome(url: URL, locale: Locale): Response {
  ...
  <main>${renderMarketSections(locale)}${renderGrowthAndMonetization(locale)}</main>`;
